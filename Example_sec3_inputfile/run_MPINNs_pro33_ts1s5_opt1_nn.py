...
# array_c = np.array([16, 32, 64, 128, 256, 512])
array_c = np.array([64, 128])
array_nn  = np.array([923,933,943])
...
## Reg.
for i_nn in array_nn:
	directory = './Output_MPINNs_examples/ds12_pro33_reg/MPINN_pro33_ts1s5_n'+str(i_nn)+'_opt1_reg2_1e5/'  # test main_PINN_CAD_v3 without STD
	src_dir   = './main_MPINNs.py'          # Recall the code
	...
	for ii in range(0,num_data):
		...
		for i_s in rand_seed:
			os.system("python {0} {1} {2} {3} \
				...
				12 1 \
				912 912 {6} 1 1 1\
				1 0 50000 0.0001 2 1.e-5 'standard' 33 \
				2 {5}".format(src_dir,directory,i_s,N_s,num_c,rand_seed_mea,i_nn))
