...
class model_CAD_seq:
    def __init__(self, X_h, Y_h, X_k, Y_k, X_c, Y_c, X_f, Y_f, X_fc, Y_fc,
        ...
        # File to record loss convergence
        self.f  = f  
        ...
        self.hbNs_pred  = self.net_hbNs(self.x1_hbNs_tf, self.x2_hbNs_tf) # v1*phi...
        ...
        ## Evaluate loss      
        '''
        # reduce_mean(X)  : mean over the matrix
        # reduce_mean(X,0): mean over each column
        # reduce_mean(X,1): mean over each rows       
        '''
        ...
        self.loss_pde_cD   = tf.reduce_mean(tf.square(self.YcbD_tf - self.cbD_pred))

        self.loss_pde_fc   = tf.reduce_mean(tf.square(self.Yfc_tf - self.fc_pred))
        self.loss_pde_cN1  = tf.reduce_mean(tf.square(self.YcbN1_tf - self.cbN1_pred))
        self.loss_pde_cN2  = tf.reduce_mean(tf.square(self.YcbN2_tf - self.cbN2_pred))
                
        self.loss_khr = self.loss_function_khr()         
        self.loss = self.loss_function() + self.reg_term
        
        self.loss_hx  = self.loss_h + self.loss_pde_D
        self.loss_cx  = self.loss_c + self.loss_pde_cD
        self.loss_fx  = self.coe_nor_k2 * (self.coe_nor_h2 * self.loss_pde_f + self.loss_pde_N + self.coe_nor_h2 * self.loss_pde_Ns)
        self.loss_fcx = self.coe_nor_c2 * (self.coe_nor_k2 * self.coe_nor_h2 * self.loss_pde_fc + self.loss_pde_cN1 + self.loss_pde_cN2)

        # Record loss
        self.rloss = []
        self.rloss_batch = [] # min-batch loss
        ...
        if type_op == 1:
            # L-BFGS: better accuracy for smooth case
            ...
            # sequent: include both c and khr; sequent 1: only with khr; sequent 2: only with c; 
            if flag_solv == 'sequent':
                self.optimizer_khr = tf.contrib.opt.ScipyOptimizerInterface( self.loss_khr,
                                                                        ...
                                                                        options = {'maxiter': 50000,
                                                                                  ...
                                                                                  'maxls': 50,
                                                                                  'ftol' : 1.0e-10})
                self.optimizer_c = tf.contrib.opt.ScipyOptimizerInterface( self.loss_cx,
                                                                        var_list=var_list3,
                                                                        method  = 'L-BFGS-B', 
                                                                        options = {'maxiter': 50000,
                                                                                  'maxfun': 50000,
        ...
        elif type_op == 7 or type_op == 72:
            # Adam
            self.optimizer_1 = tf.train.AdamOptimizer(learning_rate=learn_rate, beta1=0.9, beta2=0.999, epsilon=1e-08).minimize(self.loss,var_list=var_opt) # Verified, useful.
    ...
    def callback(self,loss,loss_k,loss_h,loss_c,loss_f,loss_fc):
        ...
        if it % num_print_opt == 0:
            print('Loss: {0:.3e}, Loss_k: {1:.3e}, Loss_h: {2: .3e}, Loss_c: {3: .3e}, Loss_f: {4:.3e}, Loss_fc: {5:.3e}'.format(loss,loss_k,loss_h,loss_c,loss_f,loss_fc)) 
            ########## record loss ############
    ...
    def callback_fast(self,loss,loss_k,loss_h,loss_c):
        num_print_opt = self.num_print_opt
        it = self.index_opt
        ########## record loss ############
        self.rloss.append(loss)
    ...
    def callback_khr(self,loss,loss_k,loss_h,loss_c,loss_f):
        num_print_opt = self.num_print_opt
        it = self.index_opt
        ########## record loss ############
        self.rloss.append(loss)
        ########## record loss ############            
        if it % num_print_opt == 0:
            print('Loss: {0:.3e}, Loss_k: {1:.3e}, Loss_h: {2: .3e}, Loss_c: {3: .3e}, Loss_f: {4:.3e}'.format(loss,loss_k,loss_h,loss_c,loss_f))
            ########## record loss ############
            self.rloss_k.append(loss_k)
            self.rloss_h.append(loss_h)
            self.rloss_c.append(loss_c)
    ...
    def train(self,type_op,batchs,nIter,flag_solv,num_print_opt): 
        ...
        if type_op == 1:
            ...
            if flag_solv == 'standard':
                ...
                if flag_solv == 'sequent':
                    '''Train loss_khr'''
                    self.optimizer_khr.minimize(self.sess, 
                               feed_dict = tf_dict,         
                               fetches = [self.loss,self.loss_k,self.loss_h,self.loss_c], 
                               loss_callback = self.callback_fast) 
 
                    loss_end, loss_khr, loss_k, loss_h, loss_f = self.sess.run([self.loss,self.loss_khr, self.loss_k,self.loss_h,self.loss_pde_f],feed_dict=tf_dict)
                    print('train: #{} Darcy, loss: {}, loss_khr: {}, loss_k: {}, loss_h: {}, loss_f: {}'.\
                        format(idex,loss_end,loss_khr,loss_k,loss_h,loss_f),file=f)
                    idex += 1

                if flag_solv == 'sequent':           
                    self.optimizer_c.minimize(self.sess, 
                               feed_dict = tf_dict,         
                               fetches = [self.loss,self.loss_k,self.loss_h,self.loss_c], 
                               loss_callback = self.callback_fast) 
                    loss_end, loss_k, loss_h, loss_c, loss_f, loss_fc = self.sess.run([self.loss,self.loss_k, self.loss_h,self.loss_c,self.loss_pde_f,self.loss_pde_fc],feed_dict=tf_dict)                       
                    
                    print('train: #{} C, loss: {}, loss_k: {}, loss_h: {}, loss_c: {}, loss_f: {}, loss_fc: {}'.\
                        format(idex,loss_end,loss_k,loss_h,loss_c,loss_f,loss_fc),file=f)
                    idex += 1
                ...
                self.optimizer.minimize(self.sess, 
                            ...
                            loss_callback = self.callback_fast) 
                loss_end, loss_k, loss_h, loss_c, loss_f, loss_fc = self.sess.run([self.loss,self.loss_k, self.loss_h,self.loss_c,self.loss_pde_f,self.loss_pde_fc],feed_dict=tf_dict)
                print('train: #{} CAD, loss: {}, loss_k: {}, loss_h: {}, loss_c: {}, loss_f: {}, loss_fc: {}'.\
                    format(idex,loss_end,loss_k,loss_h,loss_c,loss_f,loss_fc),file=f)
                idex += 1
        elif type_op == 7 or type_op == 72:
            self.num_print_opt = num_print_opt  
            
            # 2: # of batchs; 21: # of data in each batch; 22: # # of data in each batch and all data for measurement          
            if batchs == 0:
                # Run Adam optimizer
                ...
                for it in range(nIter):  # If nIter = 0, not run this sess
                    ...
                    if it % num_print_opt == 0:
                        loss_k,loss_h,loss_c,loss_f,loss_fc = self.sess.run([self.loss_k,self.loss_h,
                            self.loss_c,self.loss_pde_f,self.loss_pde_fc],feed_dict=tf_dict)
                        elapsed = time.time() - start_time

                        print('It: {0: d}, Time: {1: .2f} , Loss: {2: .3e}, Loss_k: {3: .3e}, Loss_h: {4: .3e}, Loss_c: {5: .3e}'.format(it, elapsed, loss_value, loss_k, loss_h, loss_c))
                        print('--- Loss_f: {0: .3e}, Loss_fc: {1: .3e}'.format(loss_f,loss_fc))         
                        ########## record loss ############
                        self.rloss_k.append(loss_k)
                        self.rloss_h.append(loss_h)
                        self.rloss_c.append(loss_c)
                ...
                if type_op == 72: 
                    ...
                    batch_size_c = self.x1_c.shape[0]                    
                    batch_size_cbD  = self.x1_cbD.shape[0]
                
                    batch_size_f    = sub_batch(self.x1_f.shape[0], batchs, 22)
                    batch_size_hbN  = sub_batch(self.x1_hbN.shape[0], batchs, 22)
                    batch_size_hbNs = sub_batch(self.x1_hbNs.shape[0], batchs, 22)

                    batch_size_fc = sub_batch(self.x1_fc.shape[0], batchs, 22)
                    batch_size_cbN1 = sub_batch(self.x1_cbN1.shape[0], batchs, 22)
                    batch_size_cbN2 = sub_batch(self.x1_cbN2.shape[0], batchs, 22)
                ...
                for it in range(nIter):  # If nIter = 0, not run this sess
                    ...
                    randidx_c = np.random.randint(int(self.x1_c.shape[0]), size=batch_size_c)
                    randidx_fc = np.random.randint(int(self.x1_fc.shape[0]), size=batch_size_fc)
                    randidx_cbD = np.random.randint(int(self.x1_cbD.shape[0]), size=batch_size_cbD)
                    randidx_cbN1 = np.random.randint(int(self.x1_cbN1.shape[0]), size=batch_size_cbN1)
                    randidx_cbN2 = np.random.randint(int(self.x1_cbN2.shape[0]), size=batch_size_cbN2) 
                    ...
                    tf_dict_batch = {self.x1_k_tf: self.x1_k[randidx_k,:], self.x2_k_tf: self.x2_k[randidx_k,:], self.Yk_tf: self.Y_k[randidx_k,:],
                               ...
                               self.x1_c_tf: self.x1_c[randidx_c,:], self.x2_c_tf: self.x2_c[randidx_c,:], self.Yc_tf: self.Y_c[randidx_c,:],
                               self.x1_f_tf: self.x1_f[randidx_f,:], self.x2_f_tf: self.x2_f[randidx_f,:], self.Yf_tf: self.Y_f[randidx_f,:],
                               self.x1_fc_tf: self.x1_fc[randidx_fc,:], self.x2_fc_tf: self.x2_fc[randidx_fc,:], self.Yfc_tf: self.Y_fc[randidx_fc,:],
                               ...
                               self.x1_cbD_tf: self.x1_cbD[randidx_cbD,:], self.x2_cbD_tf: self.x2_cbD[randidx_cbD,:], self.YcbD_tf: self.Y_cbD[randidx_cbD,:],
                               self.x1_cbN1_tf: self.x1_cbN1[randidx_cbN1,:], self.x2_cbN1_tf: self.x2_cbN1[randidx_cbN1,:], self.YcbN1_tf: self.Y_cbN1[randidx_cbN1,:],
                               self.x1_cbN2_tf: self.x1_cbN2[randidx_cbN2,:], self.x2_cbN2_tf: self.x2_cbN2[randidx_cbN2,:], self.YcbN2_tf: self.Y_cbN2[randidx_cbN2,:]}
                
                    _, loss_batch = self.sess.run([self.optimizer_1, self.loss], tf_dict_batch)
                    ...
                    if it % num_print_opt == 0:
                        ...
                        loss_k_bch, loss_h_bch, loss_c_bch, loss_f_bch, loss_fc_bch = self.sess.run([self.loss_k, self.loss_h, 
                            self.loss_c, self.loss_pde_f,self.loss_pde_fc], tf_dict_batch)
                        elapsed = time.time() - start_time
                        ...
                        # Full-batch
                        loss_value,loss_k,loss_h,loss_c,loss_f,loss_fc = self.sess.run([self.loss,self.loss_k,self.loss_h,
                            self.loss_c,self.loss_pde_f,self.loss_pde_fc],feed_dict=tf_dict)

                        print('*Full Batch - Loss: {0: .3e}, Loss_k: {1: .3e}, Loss_h: {2: .3e}, Loss_c: {3: .3e}, Loss_f: {4: .3e}, Loss_fc: {5: .3e}'.format(loss_value, 
                            loss_k, loss_h, loss_c,loss_f,loss_fc))                         
            ...
            loss_end, loss_k, loss_h, loss_c, loss_f, loss_fc = self.sess.run([self.loss,self.loss_k, self.loss_h,self.loss_c,self.loss_pde_f,self.loss_pde_fc],feed_dict=tf_dict)
            print('train Adam, epoch: {}, loss: {}, loss_k: {}, loss_h: {}, loss_c: {}'.\
                format(it,loss_end,loss_k,loss_h,loss_c),file=f)
            print('**** loss_f: {}, loss_fc: {}'.format(loss_f,loss_fc),file=f)
            error_k, error_h, error_c = self.error_test_l2(self.ref_xm)
            print('** rl2_k: {0:.3e}, rl2_h: {1:.3e}, rl2_c: {2: .3e}'.format(error_k,error_h,error_c),file=f)
            ...
            self.optimizer_2.minimize(self.sess, 
                                ...
                                loss_callback = self.callback_fast) 
            loss_end, loss_k, loss_h, loss_c, loss_f, loss_fc = self.sess.run([self.loss,self.loss_k, self.loss_h,self.loss_c,self.loss_pde_f,self.loss_pde_fc],feed_dict=tf_dict)
            print('train BFGS, epoch: {}, loss: {}, loss_k: {}, loss_h: {}, loss_c: {}, loss_f: {}, loss_fc: {}'.\
                format(self.index_opt,loss_end,loss_k,loss_h,loss_c,loss_f,loss_fc),file=f)            
            ...
            if batchs == 0:
                ...
                if type_op == 2 or type_op == 21:                    
                    ...
                    batch_size_c = sub_batch(self.x1_c.shape[0], batchs, type_op)
                    batch_size_cbD  = sub_batch(self.x1_cbD.shape[0], batchs, type_op)
                elif type_op == 22: # Keey all measurement data
                    batch_size_k = self.x1_k.shape[0]
                    batch_size_h = self.x1_h.shape[0]
                    batch_size_hbD = self.x1_hbD.shape[0]
                ...
                batch_size_fc = sub_batch(self.x1_fc.shape[0], batchs, type_op)
                ...
                batch_size_cbN1 = sub_batch(self.x1_cbN1.shape[0], batchs, type_op)
                batch_size_cbN2 = sub_batch(self.x1_cbN2.shape[0], batchs, type_op)
    ...
    def out_loss_components(self):
        ...
        loss_end_loss_fc  = self.sess.run(self.loss_pde_fc,feed_dict=tf_dict)
        loss_end_loss_cD  = self.sess.run(self.loss_pde_cD,feed_dict=tf_dict)
        loss_end_loss_cN1 = self.sess.run(self.loss_pde_cN1,feed_dict=tf_dict)
        loss_end_loss_cN2 = self.sess.run(self.loss_pde_cN2,feed_dict=tf_dict)
