    ...
    def train(self,type_op,batchs,nIter,flag_solv,num_print_opt): 
        ...
        if type_op == 1:
            ...
            if flag_solv == 'standard':
                ...
                print('*** rl2_k: {0:.3e}, rl2_h: {1:.3e}'.format(error_k,error_h))
            ...
            if batchs == 0: # Vanilla
                ...
                for it in range(nIter):  # If nIter = 0, not run this sess
                    ...
                    if it % num_print_opt == 0:
                        ...
                        loss_k_bch, loss_h_bch, loss_f_bch = self.sess.run([self.loss_k, self.loss_h, self.loss_pde_f], tf_dict_batch)
                        elapsed = time.time() - start_time

                        # Mini-batch
                        print('It: {0: d}, Time: {1: .2f} , Loss: {2: .3e}, Loss_k: {3: .3e}, Loss_h: {4: .3e}, Loss_f: {5: .3e}'.format(it, elapsed, loss_batch, loss_k_bch, loss_h_bch, loss_f_bch)) 

                        # Full-batch
                        loss_value, loss_k, loss_h, loss_pde_f = self.sess.run([self.loss,self.loss_k,self.loss_h, self.loss_pde_f],feed_dict=tf_dict)
                        print('* Full Batch - Loss: {0: .3e}, Loss_k: {1: .3e}, Loss_h: {2: .3e}, Loss_f: {3: .3e}'.format(loss_value, loss_k, loss_h, loss_pde_f)) 
