...
def sub_batch(num_all, batch, type):
    if type == 21 or type == 22:
        ...
        if num_all < batch * 16:  # ensure at least 16 data per training
            num_batch = 16
        else:
...
class model_Reg:
    ...
    def __init__(self, X_h, Y_h, X_k, Y_k, X_c, Y_c, X_f, Y_f, X_fc, Y_fc,
        ...
        '''
        # QH: But why use Xavier initialization
        # See note, preserve the variance...
        '''
        ...
        self.sess = tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) #standard setting
    ...
    def callback(self,loss,loss_k,loss_h,loss_c):
    ...
    def train(self,type_op,batchs,nIter,flag_solv,num_print_opt): 
        ...
        if type_op == 1:
            ...
            if flag_solv == 'standard':
                ...
                print('Not implement yet',file=f)
            ...
            if batchs == 0: # Vanilla
                ...
                for it in range(nIter):  # If nIter = 0, not run this sess
                    ...
                    if it % num_print_opt == 0:
                        ...
                        loss_k,loss_h,loss_c = self.sess.run([self.loss_k,self.loss_h,self.loss_c],feed_dict=tf_dict)
                ...
                if type_op == 72:                 
                    batch_size_k = sub_batch(self.x1_k.shape[0], batchs, 22)
                    batch_size_h = sub_batch(self.x1_h.shape[0], batchs, 22)
                    batch_size_c = sub_batch(self.x1_c.shape[0], batchs, 22)
                    batch_size_hbD = sub_batch(self.x1_hbD.shape[0], batchs, 22)
                    batch_size_cbD  = sub_batch(self.x1_cbD.shape[0], batchs, 22)
                ...
                for it in range(nIter):  # If nIter = 0, not run this sess
                    ...
                    # Print
                    if it % num_print_opt == 0:
                        loss_k_bch, loss_h_bch, loss_c_bch = self.sess.run([self.loss_k, self.loss_h, self.loss_c], tf_dict_batch)
                        elapsed = time.time() - start_time

                        # Mini-batch
                        print('It: {0: d}, Time: {1: .2f}, Loss: {2: .3e}, Loss_k: {3: .3e}, Loss_h: {4: .3e}, Loss_c: {5: .3e}'.format(it, elapsed,loss_batch, loss_k_bch, loss_h_bch, loss_c_bch)) 

                        # Full-batch
                        loss_value, loss_k,loss_h,loss_c = self.sess.run([self.loss,self.loss_k,self.loss_h,self.loss_c],feed_dict=tf_dict)
                        print('* Full batch - Loss: {0: .3e}, Loss_k: {1: .3e}, Loss_h: {2: .3e}, Loss_c: {3: .3e}'.format(loss_value,loss_k,loss_h,loss_c)) 
            ...
            loss_end, loss_k, loss_h, loss_c = self.sess.run([self.loss,self.loss_k, self.loss_h,self.loss_c],feed_dict=tf_dict)
            error_k, error_h, error_c = self.error_test_l2(self.ref_xm)
            ...
            print('train BFGS, epoch: {}, loss: {}, loss_k: {}, loss_h: {}, loss_c: {}'.\
                format(self.index_opt,loss_end,loss_k,loss_h,loss_c),file=f)
            print('** rl2_k: {0:.3e}, rl2_h: {1:.3e}, rl2_c: {2: .3e}'.format(error_k,error_h,error_c))
    ...
    def out_loss_components(self):
        ...
        loss_end_loss_k_sca = para_k  * coe_std_k2 * loss_end_loss_k
        loss_end_loss_h_sca = para_h  * coe_std_h2 * loss_end_loss_h
        loss_end_loss_c_sca = para_c  * coe_std_c2 * loss_end_loss_c

        # print('Loss_k: {0: .3e}, Loss_h: {1: .3e}, Loss_c: {0: .3e}'\
        #   .format(loss_end_loss_k,loss_end_loss_h,loss_end_loss_c),file=f)

        print('Scaled Loss_k: {0: .3e}, Loss_h: {1: .3e}, Loss_c: {2: .3e}'\
          .format(loss_end_loss_k_sca,loss_end_loss_h_sca,loss_end_loss_c_sca),file=f)
                     
        print('{0:.3e} {1:.3e} {2:.3e}'\
          .format(loss_end_loss_k, loss_end_loss_h, loss_end_loss_c),file=f1_loss)

        print('{0:.3e} {1:.3e} {2:.3e}'\
          .format(loss_end_loss_k_sca, loss_end_loss_h_sca, loss_end_loss_c_sca),file=f1_loss2)
    ...
    def net_fc(self, x1, x2):
        ...
        # ## --------------------------------
        # # Diffusion
        # f_1d = phi*Dw*tau*tf.gradients(c_x1, x1)[0] * beta[0] 
        # f_2d = phi*Dw*tau*tf.gradients(c_x2, x2)[0] * beta[1]

        # # Dipersive
        # f_1p = tf.gradients(aL * v_l2 * c_x1, x1)[0] * beta[0] 
        # f_2p = tf.gradients(aT * v_l2 * c_x2, x2)[0] * beta[1]

        # fc_R = f_1d + f_2d + f_1p + f_2p         
        # ## --------------------------------
