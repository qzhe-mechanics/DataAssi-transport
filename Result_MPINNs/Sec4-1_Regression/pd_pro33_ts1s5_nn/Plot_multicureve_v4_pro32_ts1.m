...
mainFolder = {  'CADn_v6s1_pro33_ts1s5_n913_opt1',...
                'CADn_v6s1_pro33_ts1s5_n923_opt1',...
                'CADn_v6s1_pro33_ts1s5_n914_opt1',...
                'CADn_v6s1_pro33_ts1s5_n924_opt1'};
            
name_test = 'CADn_v6s2_pro33_ts1s5_nn';

type_pro  = 3; % 1: plot k, 2: plot h; 3: C
type_xlab = 4;  % The x-axis...
type_plot = 2; % 1: only mean; 2: error plot
if_log  = 0;

num_dataset = numel (mainFolder);
N_s = 5; % Number of NNs initialization
x = [16	32	64	128	256	512]; % Measurement point

marker = {'-ob','--xb','-or','--xr','-ok','-+k'};
marker_err = {'-b','--b','-r','--r','-k','--k'};
% marker = {'-ok','-*r','-xb','-^m','->'};
% marker_err = {'--k','--r','--b','--m','--'};
lstr_leng = {'hidden layers $[-32-32-]$', 'hidden layers $[-32-32-32-]$', 'hidden layers $[-64-64-]$', 'hidden layers $[-64-64-64-]$'};
