...
QiZhi He, David Brajas-Solano, Guzel Tartakovsky, Alexandre M. Tartakovsky, "Data Assimilation based on Physics-Informed Neural Networks for Subsurface Transport"

Correspondence: Alexandre.Tartakovsky@pnnl.gov; qizhi.he@pnnl.gov
"""
...
'''System Input'''
path_f   = sys.argv[1]      if len(sys.argv) > 1 else './Output/test_mac/td12_pro2_opt2i5k_lr1e3_k36_base3/'
seed_num = int(sys.argv[2]) if len(sys.argv) > 2 else 111
N_s      = int(sys.argv[3]) if len(sys.argv) > 3 else 1    # Initialization of NNs
N_k      = int(sys.argv[4]) if len(sys.argv) > 4 else 36   # Measurement of Conductivity
N_h      = int(sys.argv[5]) if len(sys.argv) > 5 else 36   # Measurement of Pressure head
...
# NN Activation function (1: tanh; 2.ReLu;) (Not Implemented Yet...)
...
type_op    = int(sys.argv[22])  if len(sys.argv) > 22 else 2 
batchs     = int(sys.argv[23])  if len(sys.argv) > 23 else 0        # number of batchs
num_epochs = int(sys.argv[24])  if len(sys.argv) > 24 else 5000     # number of itrations
...
flag_pro   = int(sys.argv[29])  if len(sys.argv) > 29 else 2    
sys_id = 29
sys_id += 1  
...
if flag_data == 12:
    ...
    path_data = './Data_mea/data_smooth_k01_normal/' # Correlation lenght = 0.1
elif flag_data == 53:
    path_data = './Data_mea/data_smooth_k03_normal/' # Correlation lenght = 0.3
elif flag_data == 57:
    path_data = './Data_mea/data_smooth_k07_normal/' # Correlation lenght = 0.7
...
if __name__ == "__main__": 
    ...
    path_coor = path_f+'record_coor'+'_k'+str(N_k)+'_h'+str(N_h)+'_f'+str(N_f)+'_c'+str(N_c)+'_fc'+str(N_fc)+'/'
    
    if not os.path.exists(path_coor):
        os.makedirs(path_coor)
    f_Xk  = open(path_coor+'record_Xk.out', 'a+')
    f_Xh  = open(path_coor+'record_Xh.out', 'a+')
    f_Xc  = open(path_coor+'record_Xc.out', 'a+')
    f_X_f = open(path_coor+'record_Xf.out', 'a+')
    f_X_fc = open(path_coor+'record_Xfc.out', 'a+')

    mat_k = np.matrix(X_k)
    for line in mat_k:
        np.savetxt(f_Xk, line, fmt='%.8f')
    mat_h = np.matrix(X_h)
    for line in mat_h:
        np.savetxt(f_Xh, line, fmt='%.8f') 
    mat_c = np.matrix(X_c)              
    for line in mat_c:
        np.savetxt(f_Xc, line, fmt='%.8f')    
    mat_f = np.matrix(X_f)
    for line in mat_f:
        np.savetxt(f_X_f, line, fmt='%.8f') 
    mat_fc = np.matrix(X_fc)              
    for line in mat_fc:
        np.savetxt(f_X_fc, line, fmt='%.8f') 
    ...
    f_Xstar  = open(path_coor+'record_Xstar.out', 'a+')
    mat_x = np.matrix(X_star)
    for line in mat_x:
        np.savetxt(f_Xstar, line, fmt='%.8f')
    f_Xstar.close()
    ...
    for i_loop in range(0,N_s):
        ...
        if IF_plot == 1:
            ...
            if flag_pro == 1:
                if_plot_k, if_plot_h, if_plot_c = 1, 1, 1
                if_plot_fh, if_plot_fc = 1, 1
            elif flag_pro == 2:
                if_plot_k, if_plot_h, if_plot_c = 1, 1, 0
                if_plot_fh, if_plot_fc = 1, 0
            elif flag_pro == 31:
                if_plot_k, if_plot_h, if_plot_c = 1, 0, 0
                if_plot_fh, if_plot_fc = 0, 0
            elif flag_pro == 32:
                if_plot_k, if_plot_h, if_plot_c = 0, 1, 0
                if_plot_fh, if_plot_fc = 0, 0
            elif flag_pro == 33:
                if_plot_k, if_plot_h, if_plot_c = 0, 0, 1
                if_plot_fh, if_plot_fc = 0, 0
            elif flag_pro == 34:
                if_plot_k, if_plot_h, if_plot_c = 1, 1, 0
                if_plot_fh, if_plot_fc = 0, 0
            ...
            if if_plot_fh == 1:
                # Record
                fh_pred = model.predict_f(X_star)
                f_fh_pred  = open(path_tf+'record_fh_pred.out', 'a+')
                mat_fh = np.matrix(fh_pred)
                for line in mat_fh:
                    np.savetxt(f_fh_pred, line, fmt='%.8f')
                f_fh_pred.close()

                f_plot = griddata(X_star, fh_pred.flatten(), (XX, YY), method='cubic')
            ...
            # Plot f_advect-dispersion
            if if_plot_fc == 1:
                # Record
                fc_pred = model.predict_fc(X_star)
                f_fc_pred  = open(path_tf+'record_fc_pred.out', 'a+')
                mat_fc = np.matrix(fc_pred)
                for line in mat_fc:
                    np.savetxt(f_fc_pred, line, fmt='%.8f')
                f_fc_pred.close()
